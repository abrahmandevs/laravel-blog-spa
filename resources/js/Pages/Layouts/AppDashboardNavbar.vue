<script setup>
    ...
    const data = usePage();
    const auth = data.props.flash.auth;
...
<template>
	<div class="mx-auto px-3 sm:px-6 bg-[#262d33] w-full h-[50px] flex items-center justify-betwee shadow" @click.stop="manuToogelClose">
		...
		<div class="grow flex items-center gap-x-2 justify-end">
			...
			<MenuDropdown position="left" :labelIcon="false" class="text-white" buttonClass="hover:bg-gray-700 h-8 w-8 rounded-full">
				...
				<div class="divide-y divide-gray-300 max-h-80 overflow-y-auto transition-all duration-300">
					...
					<div class="relative flex items-center gap-x-4 overflow-hidden p-4">
						<img v-if="auth" class="h-11 w-11 rounded-full object-cover" :src="auth.image" alt="" />
						<div class="space-y-2">
			...
			<MenuDropdown position="left" :labelIcon="false" class="text-white" buttonClass="hover:bg-gray-700 h-8 w-8 rounded-full">
				<template #button>
					<UserIcon v-if="!auth" class="block p-1.5" aria-hidden="true" />
					<img v-if="auth" class="block object-cover" :src="auth.image" alt="" />
				</template>
				<div class="divide-y divide-gray-300">
					<div class="" v-if="auth">
						<Link v-if="auth" :href="route('profile')" class="block px-3 py-1.5 text-black hover:text-gray-500 transition-all duration-300">Profile</Link>

						<Link v-if="auth" href="/" class="block px-3 py-1.5 text-black hover:text-gray-500 transition-all duration-300">Settings</Link>

						<Link v-if="auth" href="/" class="block px-3 py-1.5 text-black hover:text-gray-500 transition-all duration-300">Analisys</Link>
					</div>

					<div class="" v-if="auth">
						<Link :href="route('logout')" method="post"
                                        as="button" class="block px-3 py-2 text-black hover:text-gray-500 transition-all duration-300">
